...
import hackathon.co.kr.neopen.sdk.broadcastreceiver.BTDuplicateRemoveBroadcasterReceiver;
...
/**
 * Created by KIM on 2018-02-20.
 * This class is almost same BTAdt. But OfflineConnectedThread(using Piped Stream) is added.
 */

public class BTOfflineAdt implements IPenAdt {      //[2018.03.05] Stroke Test
    private static BTOfflineAdt myInstance = null;
...
// Name for the SDP record when creating server socket
    ...
    private OfflineConnectedThread mOfflineConnectionThread;       //[2018.03.05] Stroke Test
     ...
     * @return the instance
     */
    public synchronized static BTOfflineAdt getInstance()
    {
        if ( myInstance == null )
        {
            if ( myInstance == null )
            {
                myInstance = new BTOfflineAdt();
            }
        }
    ...
    public synchronized void connect( String address, boolean isFreePass)
    {
        ...
        if ( !ret )
        {
            NLog.e( "[BTOfflineAdt] Your device is not allowed." );
        ...
        NLog.i( "[BTOfflineAdt] connect device : " + address );
...
//			throw new ProtocolNotSupportedException( "getPressSensorType( ) is supported from protocol 2.0 !!!");
    ...
    {
        NLog.i( "[BTOfflineAdt] disconnect device" );
    ...
    {
        NLog.i( "[BTOfflineAdt] stop listen" );
    ...
    {
        return true;//mac.startsWith( ALLOWED_MAC_PREFIX ) && !mac.startsWith( DENIED_MAC_PREFIX );
    }
...
//        }
    ...
    private class ListenThread extends Thread
    {
        ...
        public ListenThread()
        {
            NLog.i( "[BTOfflineAdt] ListenThread startup" );
            setNewServerSocket();
        }
        ...
        public void setNewServerSocket()
        {
            ...
            try
            {
                ...
                NLog.d( "[BTOfflineAdt] ListenThread new BT ServerSocket assigned" );
            }
            catch ( IOException e )
            {
                NLog.e( "[BTOfflineAdt] ListenThread new BT ServerSocket assign fail", e );
            }
        ...
        public void run()
        {
            NLog.i( "[BTOfflineAdt] ListenThread running" );
            ...
            while ( mmServerSocket != null )
            {
                try
                {
                    NLog.d( "[BTOfflineAdt] ListenThread ready to connection" );

                    if ( mmServerSocket != null )
                    {
                        NLog.d( "[BTOfflineAdt] Wait new connection" );
                        socket = mmServerSocket.accept();
                    }
                }
                catch ( IOException e )
                {
                    NLog.e( "[BTOfflineAdt] ListenThread fail to listen socket", e );
                    break;
                }

                if ( socket != null )
                {
                    synchronized ( BTOfflineAdt.this )
                    {
                        boolean ret;
                        try {
                        ...
                        NLog.d( "[BTOfflineAdt] ListenThread success to listen socket : " + socket.getRemoteDevice().getAddress() );
        ...
        public void cancel()
        {
            NLog.d( "[BTOfflineAdt] ListenThread cancel" );
            ...
            {
                NLog.e( "[BTOfflineAdt] ListenThread fail to close server socket", e );
            }
        }
    }
    ...
    private class ConnectThread extends Thread
    {
        ...
        public ConnectThread( BluetoothDevice device )
        {
            ...
            {
                NLog.e( "[BTOfflineAdt/ConnectThread] Socket Type : create() failed", e );
            }
        ...
        public void run()
        {
            NLog.d( "[BTOfflineAdt/ConnectThread] ConnectThread STARTED" );
            ...
            try
            {
                ...
                mmSocket.connect();
                NLog.d( "[BTOfflineAdt/ConnectThread] success to connect socket" );
            }
            catch ( IOException e )
            {
                NLog.e( "[BTOfflineAdt/ConnectThread] fail to connect socket.", e );
                ...
                {
                    NLog.e( "[BTOfflineAdt/ConnectThread] fail to close socket", e2 );
                }
        ...
        public void cancel()
        {
            NLog.d( "[BTOfflineAdt/ConnectThread] cancel()" );
            ...
            {
                NLog.e( "[BTOfflineAdt/ConnectThread] fail to close socket", e );
            }
        }
    }
...
//			}
//		} );
        NLog.i( "[BTOfflineAdt] bindConnection by BluetoothSocket : " + socket.getRemoteDevice().getAddress() + ";mProtocolVer=" + mProtocolVer + ";COD=" + socket.getRemoteDevice().getBluetoothClass().getDeviceClass() + ";getBluetoothClass=" + socket.getRemoteDevice().getBluetoothClass().toString() );

        mConnectionThread = new ConnectedThread(mProtocolVer);
        mOfflineConnectionThread = new OfflineConnectedThread();   //[2018.03.05] Stroke Test

        if ( mConnectionThread.bind( socket ) )
        {
            mConnectionThread.start();
            mOfflineConnectionThread.start();
            this.onBinded();
        }
        else
        {
    ...
    private void onLostConnection()
    {
        ...
        NLog.d( "[BTOfflineAdt/ConnectThread] onLostConnection mIsRegularDisconnect="+mIsRegularDisconnect );
...
//		penAddress = address;
    ...
    public class ConnectedThread extends Thread implements IConnectedThread
    {
        ...
        {
            NLog.d( "[BTOfflineAdt/ConnectedThread] stopRunning()" );
            if(processor != null)
            {
        ...
        public boolean bind( BluetoothSocket socket )
        {
            ...
            {
                NLog.e( "[BTOfflineAdt/ConnectedThread] temporary sockets is not created", e );
            }
        ...
        {
            NLog.d( "[BTOfflineAdt/ConnectedThread] unbind() isRegularDisconnect="+isRegularDisconnect );
            ...
            if ( mmSocket != null )
            {
                ...
                {
                    NLog.e( "[BTOfflineAdt/ConnectedThread] socket closing fail at unbind time.", e );
                }
            }
            else
            {
                NLog.d( "[BTOfflineAdt/ConnectedThread] socket is null!!" );
            }
        ...
        public void run()
        {
            NLog.d( "[BTOfflineAdt/ConnectedThread] STARTED" );
        ...
        public void read()
        {
            ...
            {
                NLog.d( "[BTOfflineAdt/ConnectedThread] read run!!!!!!!!!!!");
                try
                {
                    bytes = mmInStream.read( buffer );
//                    bytes = mPipedInputStream.read( buffer );
                    NLog.d( "[BTOfflineAdt/ConnectedThread] TimeCheck read bytes="+bytes );
                    ...
                    NLog.d( "[BTOfflineAdt/ConnectedThread] read bytes data = "+ buff.toString() );
                ...
                {
                    NLog.e( "[BTOfflineAdt/ConnectedThread] ConnectedThread read IOException occured.", e );
                    this.stopRunning();
                    break;
                }
            }
        ...
        public void write( byte[] buffer )
        {
            ...
            {
                NLog.e( "[BTOfflineAdt/ConnectedThread] IOException during write.", e );
                this.stopRunning();
            }
        }
    ...
    /**
     * Read data from piped input stream and then send to commProcessor20.
     */
    public class OfflineConnectedThread extends Thread {
        @Override
        public void run() {
        ...
        public void read()
        {
            ...
            while ( true )
            {
                NLog.d( "[BTOfflineAdt/OfflineConnectedThread] read run!!!!!!!!!!!");
                try
                {
//                    bytes = mmInStream.read( buffer );
                    bytes = mPipedInputStream.read( buffer );
                    NLog.d( "[BTOfflineAdt/OfflineConnectedThread] TimeCheck read bytes="+bytes );
                    ...
                    NLog.d( "[BTOfflineAdt/OfflineConnectedThread] read bytes data = "+ buff.toString() );

                    if ( bytes > 0 )
                    {
                        mConnectionThread.processor.fill( buffer, bytes );
                    }
                    else if ( bytes == -1 )
                    {
                        return;
                ...
                {
                    NLog.e( "[BTOfflineAdt/OfflineConnectedThread] ConnectedThread read IOException occured.", e );
                    return;
                }
            }
        }
        ...
        public void handleMessage( Message msg )
        {
            switch ( msg.what )
            {
                ...
                case QUEUE_MSG:
                {
                    ...
                    {
                        NLog.d( "[BTOfflineAdt/mHandler] PenMsgType.PEN_DISCONNECTED" );
                        penAddress = null;
                    }
